...
export const FASE_POR_NUM_PARTIDOS: Record<
  number,
  string
> = {
  8: "octavos",
...
export function buildFirstRound(
  ...
  const parejas = pairsSeeded.filter(
    (id): id is string =>
      typeof id === "string" &&
      id.length > 0
  );
  ...
  /*
   * Los mejores seeds reciben los byes.
   * El resto se enfrenta cruzando extremos:
   *
   * 1 vs último
   * 2 vs penúltimo
   * ...
   */
  const byes = parejas.slice(
    0,
    numByes
  );

  const resto = parejas.slice(
    numByes
  );
