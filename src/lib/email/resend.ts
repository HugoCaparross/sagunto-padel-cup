...
function validateEmail(value: string): void {
  ...
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    throw new Error("Dirección de email no válida.");
  }
}
...
function validateHttpsUrl(value: string): URL {
  ...
  if (url.protocol !== "https:" && url.hostname !== "localhost") {
    throw new Error("La URL debe utilizar HTTPS.");
  }
...
export async function sendRegistrationConfirmedEmail(params: {
  ...
  const nombreSeguro = escapeHtml(params.nombre.trim());

  const torneoSeguro = escapeHtml(params.torneoNombre.trim());
...
export async function sendPartnerInviteEmail(params: {
  ...
  const signupUrl = validateHttpsUrl(params.signupUrl);

  const nombreSeguro = escapeHtml(params.invitadoPorNombre.trim());

  const torneoSeguro = escapeHtml(params.torneoNombre.trim());

  const emailSeguro = escapeHtml(to);

  const signupUrlSeguro = escapeHtml(signupUrl.toString());
