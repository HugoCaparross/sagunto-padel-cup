// Ruta: src/app/(public)/torneo/[slug]/inscribirse/page.tsx — sustituye entero al archivo actual
...
export default async function InscribirsePage({
  ...
  const { data: bolsa } = user
    ? await supabase
        .from("partner_pool")
        .select("id, categorias:categories(nombre), players(nombre, apellidos, email)")
                ...
                {bolsa.map((b) => {
                  ...
                  return (
                    <li key={b.id}>
                      ...
                      {(b.categorias as unknown as { nombre: string })?.nombre} —{" "}
                      <a href={`mailto:${jugador?.email}`} className="underline text-coral">
                        {jugador?.email}
                      </a>
                    </li>
                  );
