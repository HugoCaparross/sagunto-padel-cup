  ...
  return (
    <main className="mx-auto max-w-7xl px-5 py-12 sm:py-14">
      <header className="mb-8 border-b border-navy/10 pb-6">
        ...
        <h1 className="mt-1 font-display text-4xl font-semibold">Calendario</h1>
      ...
      <form
        method="get"
        className="mb-8 grid gap-4 border-y border-navy/10 py-5 sm:grid-cols-2"
      >
        <div>
          <label
        ...
        <ol className="divide-y divide-navy/10 border-y border-navy/10">
          {torneosFiltrados.map((torneo) => {
            ...
            return (
              <li
                key={torneo.id}
                className="border-b border-navy/10 py-5 last:border-b-0"
              >
                <Link
                  href={`/torneo/${torneo.slug}`}
                  className="block py-2 transition-colors hover:text-coral focus:outline-none focus-visible:text-coral"
                >
